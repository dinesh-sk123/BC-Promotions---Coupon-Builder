...
if( is_dir( get_parent_theme_file_path().'/bluecorona-plugin-promotions-master/') ){
        define( 'BCPATH', get_parent_theme_file_path().'/bluecorona-plugin-promotions-master/' );
    }
...
function get_promotion_query($attr)
{
    ...
    $promotions_args = ['post_type' => 'bc_promotions', 'posts_per_page' => -1, 'order' => 'DESC', 'post_status' => 'publish'];
    ...
    if ($promotion_show_value == 0) {
        $promotions_args = [
            ...
            'posts_per_page' => $attr, // Assuming $attr is set elsewhere in your code
            'order' => 'DESC',
            'post_status' => 'publish',
        ...
        if ($defalut_value == 1) {
            ...
            if ($promotion_default_checkbox_value == 2) {
                ...
                $promotions_args = ['post_type' => 'bc_promotions', 'posts_per_page' => $attr, 'order' => 'DESC', 'post_status' => 'publish', 'post__in' => $promotion_spa, 'paged' => get_query_var('paged') ? get_query_var('paged') : 1];

                return $query = new WP_Query($promotions_args);

                //coupan not show on this page
            } else {
                $promotions_args = ['post_type' => 'bc_promotions', 'posts_per_page' => $attr, 'order' => 'DESC', 'post_status' => 'publish', 'paged' => get_query_var('paged') ? get_query_var('paged') : 1];
...
function bc_include_css_js($hook)
{
    ...
    if ($current_screen->post_type == 'bc_promotions') {
        // Include CSS Libs
        wp_register_style('bc-plugin-css', plugins_url('assests/css/bootstrap.min.css', __FILE__), [], '1.0.0', 'all');
        wp_enqueue_style('bc-plugin-css');
        wp_enqueue_style('bc-colorpicker-css', plugins_url('assests/css/bootstrap-colorpicker.min.css', __FILE__), [], '1.0.0', 'all');
        wp_enqueue_style('bc-datepicker', plugins_url('assests/css/datepicker3.css', __FILE__), [], '1.0.0', 'all');
        // wp_enqueue_style('bc-fontawesome',plugins_url('assests/css/font-awesome.min.css', __FILE__), array(), '1.0.0', 'all');
        wp_enqueue_style('bc-style', plugins_url('assests/css/bc-style.css', __FILE__), [], '1.0.0', 'all');
        ...
        wp_enqueue_script('bc-image-upload-js', plugin_dir_url(__FILE__) . 'assests/js/bc-image-upload.js', ['jquery']);
    }
}
...
<?php
    ...
    $Ids = null;
    $args = ['post_type' => 'bc_promotions', 'posts_per_page' => -1, 'order' => 'DESC', 'post_status' => 'publish'];
...
function manage_promotions_columns($column_name, $id)
{
    ...
    switch ($column_name) {
        ...
        case 'promotion_expiry_date1':
            ...
            break;
        case 'updated':
            echo date('d/m/Y', strtotime($post->post_modified));
            break;
        case 'status':
...
if ($promotion_type == 'Builder') {
    ...
    $saved_date = date("m/d/Y");
    list($month, $day, $year) = explode('/', $saved_date);
    $date_for_strtotime = "$year-$month-$day";
    $last_day_of_month = date('t', strtotime($date_for_strtotime));
    $last_day_full_date = date("m/d/Y", strtotime($date_for_strtotime));
    if ($recuring == 1) {
        $formatted_date = date('m/d/Y', strtotime($saved_date));
        $date11 = date_create($formatted_date); 
        $date2 = date_create($expiration_date);
        $diff = date_diff($date11, $date2);
        $current_diff = $diff->format("%R%a");  
        if ($current_diff <= 0 && $enddate == 0) {
            $dateString = $date2->format('m/d/Y');
            ...
            $d = cal_days_in_month(CAL_GREGORIAN, $newMonth, $year1);
            date_add($date1, date_interval_create_from_date_string($d . ' days'));
            $final = date_format($date1, 'm/d/Y');     
            update_post_meta($ids, 'promotion_expiry_date1', $final);
        } elseif ($current_diff <= 0 && $recuring == 1 && $enddate == 1) {
            $last_day_of_month = date("t", strtotime(date("Y-m-d")));
            $current_day = date("d", strtotime(date("Y-m-d")));
                ...
                $newMonth = $month1 + 1;
                if ($newMonth > 12) {
                    $newMonth = 1; // If it's December, set the month to January
        ...
        } elseif ($current_diff < 0 && $enddate == 1) {

            $lastdate = date("m/t/y", strtotime(date("m/d/Y")));
            update_post_meta($ids, 'promotion_expiry_date1', $last_day_full_date);
        } 
    }
}
 elseif ($promotion_type == 'Image') {
                ...
                if ($recuring == 1) {
                    ...
                    } elseif ($current_diff2 < 0 && $enddate2 == 1) {
